...
const Login = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="flex min-h-screen w-full font-sans text-slate-800 selection:bg-blue-200 relative pt-8">
      ...
      {/* LEFT SIDE: App Innovation */}
      <div className="hidden md:flex flex-col w-1/2 bg-white relative overflow-hidden px-8 lg:px-16 py-6 z-10">
        ...
        <div className="absolute -left-28 top-0 w-[28rem] h-[28rem] bg-gradient-to-tr from-blue-200/50 to-indigo-200/30 rounded-full mix-blend-multiply filter blur-[90px] opacity-70 animate-pulse z-0"></div>

        {/* 🔥 UPDATED: TOP-LEFT LOGO (Positioned High & Clean) 🔥 */}
        ...
        <div className="flex flex-col justify-center flex-grow pt-20 lg:pt-22 relative z-10">
          ...
          <p className="text-sm text-slate-600 font-medium mb-5 leading-relaxed max-w-lg">
          ...
          {/* 🔥 UPDATED: ULTRA-PREMIUM PRO TERMINAL ENGINE BOX 🔥 */}
          <div className="mb-6 rounded-2xl overflow-hidden shadow-2xl shadow-blue-900/10 border border-slate-800 bg-slate-950 flex flex-col max-w-lg transition-all duration-300 hover:shadow-blue-500/15 group">
            {/* Terminal Header */}
          ...
          >
            <motion.div variants={itemVariants} className="flex items-start gap-3.5 p-2.5 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-black text-xs shadow-inner border border-blue-300">
                1
              </div>
              <div>
                <h4 className="text-sm font-extrabold text-slate-900">Describe Naturally & AI Auto-Mapping</h4>
                <p className="text-xs text-slate-600 font-medium mt-0.5">Explain your logic in plain English or Tanglish. Our AI instantly translates your text into precision options legs, strikes, and execution rules.</p>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="flex items-start gap-3.5 p-2.5 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 font-medium mt-0.5">The moment your backtest completes, our AI analyzes your MFE/MAE and trade sequence to generate a personalized report on exactly how to improve your strategy.</p>
              </div>
            </motion.div>

            <motion.div variants={itemVariants} className="flex items-start gap-3.5 p-2.5 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-black text-xs shadow-inner border border-green-300">
                3
              </div>
              <div>
                <h4 className="text-sm font-extrabold text-slate-900">Granular Deep Filtering Engine</h4>
                <p className="text-xs text-slate-600 font-medium mt-0.5">Slice your data with precision using 0DTE, Day-wise, Win/Loss, and Buy/Sell leg filters to find your true hidden edge.</p>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="flex items-start gap-3.5 p-2.5 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 font-medium mt-0.5">Go beyond basic PnL. We provide Profit Factor, Sortino Ratio, System Survival Probability, Kelly Sizing, Stress Level Index, Tail Ratio & Scalability.</p>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="flex items-start gap-3.5 p-2.5 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 font-medium mt-0.5">Export ledgers via PDF/CSV. Analyze performance with Heatmaps, PnL Curves, Drawdown charts, and deep AI insights in detailed backtest reports.</p>
              </div>
            </motion.div>
          </motion.div>
          ...
          <div 
            ...
            className={`mt-5 p-3.5 rounded-xl cursor-pointer transition-all border border-blue-300 bg-gradient-to-r from-blue-50 to-indigo-100 hover:border-blue-500 hover:shadow-lg flex items-center justify-between group max-w-md ${isLoading ? 'opacity-70 pointer-events-none' : ''}`}
          >
      ...
      {/* RIGHT SIDE (Sleek Modern Light Blue Theme + Full Cover Layout) */}
      <div className="w-full md:w-1/2 bg-gradient-to-br from-slate-100 via-blue-50/80 to-indigo-100/70 flex flex-col items-center justify-start p-4 md:p-6 lg:p-8 relative z-10 pt-4 pb-12 overflow-y-auto">
        
        {/* Soft Decorative Ambient Background Elements */}
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden opacity-60 pointer-events-none z-0">
           <div className="absolute top-10 right-10 w-96 h-96 bg-blue-300/40 rounded-full blur-[100px]"></div>
           <div className="absolute bottom-20 left-10 w-80 h-80 bg-indigo-300/30 rounded-full blur-[100px]"></div>
        </div>

        {/* 🔥 UPDATED: Login Card (Fits Viewport Perfectly On Load) 🔥 */}
        <div className="w-full max-w-md bg-white/95 backdrop-blur-2xl rounded-3xl shadow-[0_20px_50px_rgba(30,58,138,0.12)] p-6 lg:p-8 z-10 border border-white shrink-0 mt-1 mb-8">
          ...
          <div className="mt-5 text-center">
            <p className="text-xs text-slate-600 font-semibold">
              Don't have an account? <span className="text-blue-600 font-black hover:text-blue-800 hover:underline cursor-pointer transition-all">Sign up</span>
            </p>
          </div>
        </div>

        {/* 🔥 UPDATED: UNIQUE EXECUTIVE LIGHT-MODE SEBI REALITY CHECK CARD 🔥 */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          className="w-full max-w-md bg-white rounded-2xl border border-rose-100 shadow-xl shadow-rose-900/5 p-5 relative overflow-hidden shrink-0 mb-6 group"
        >
        ...
        {/* 🔥 UPDATED: UNIQUE EXECUTIVE LIGHT TRADER REVIEWS & INNOVATION CARD 🔥 */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5, delay: 0.1 }}
          className="w-full max-w-md bg-white rounded-2xl border border-blue-100 shadow-xl shadow-blue-900/5 p-5 shrink-0 flex flex-col justify-between"
        >
          <div>
                    ...
                    {testimonials.map((_, idx) => (
                      <button
                        ...
                        className={`w-2 h-2 rounded-full transition-all ${
                          activeReview === idx ? 'bg-blue-600 w-5' : 'bg-slate-300 hover:bg-slate-400'
                        }`}
                      />
                    ))}
                  </div>
