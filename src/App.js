...
// 🚨 NEW: React Router imports for Navigation 🚨
...
function App() {
  ...
  // user ilatha apo Home, Login, Signup page pogum. user ulla vanthuta Dashboard varum.
  return (
    <Router>
      <Routes>
        
        {/* Unauthenticated Routes */}
        <Route path="/" element={!user ? <HomeView /> : <Navigate to="/dashboard" />} />
        <Route path="/login" element={!user ? <Login onLoginSuccess={(loggedInUser) => setUser(loggedInUser)} /> : <Navigate to="/dashboard" />} />
        <Route path="/signup" element={!user ? <AuthView /> : <Navigate to="/dashboard" />} />
